...
export class UIChat extends tgx.UIController {
  ...
  private getFromPool() {
    const layout = this._layout as Layout_UIChat;
  ...
  protected onCreated(net: NetGameServer): void {
    ...
    const w = layout.chatMsgs.getComponent(Widget);
    w.bottom = 15;
    w.left = 10;
  ...
  async onUserComes(v: MsgUserComesToRoomPush) {
    const info = RoomMgr.inst.getUser(v.uid);
    this._pushChatMsg(
      `<outline width=1 color=#0><color=#33A8E9>${info?.name}(${info?.uid})</color> <color=#999999>加入了房间</color></o>`,
    );
  }
  ...
  async onUserLeaves(v: MsgUserLeavesFromRoomPush) {
    const info = RoomMgr.inst.getUser(v.uid);
    this._pushChatMsg(
      `<outline width=1 color=#0><color=#33A8E9>${info?.name}(${info?.uid})</color> <color=#999999>离开了房间</color></o>`,
    );
  }
  ...
  addChatMsg(msg: MsgChat) {
    ...
    this._pushChatMsg(
      `<outline width=1 color=#0><color=#33A8E9>${str}${msg.user.name}(${msg.user.uid})</color> <color=#FFFFFF>${msg.content}</color></o>`,
    );
  }
  ...
  async onBtnSendChat() {
    ...
    if (index != -1) {
      const headStr = content.substring(0, index);
    ...
    const ret = await this._chatNet.callApi('chat/SendChat', {
      channel: channel,
      content: content,
    });
    layout.inputChat.string = '';
