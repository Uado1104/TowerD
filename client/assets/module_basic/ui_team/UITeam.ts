...
export class UITeam extends tgx.UIController {
  ...
  protected async onCreated(args: any) {
    ...
    await UserMgr.inst.setUserIconAndName(
      UserMgr.inst.uid,
      this.layout.sprIcon,
      this.layout.lblUserName,
      ModuleDef.BASIC,
    );
    ...
    this.onButtonEvent(this.layout.btnClose, async () => {
      ...
      //@zh 如果正在匹配中，则需要取消匹配
      if (this.isMatching) {
        const ret = await LobbyMgr.inst.rpc_QuickPlayCancel();
        if (ret.isSucc) {
          this.close();
        }
      } else {
  ...
  protected onUpdate(dt: number): void {
    if (this._startMatchTime > 0) {
      const elapsedTime = Math.floor((Date.now() - this._startMatchTime) / 1000);
      if (elapsedTime < 60) {
