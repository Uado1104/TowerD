...
export class UIAnnouncement extends tgx.UIController {
  ...
  protected async onCreated(): Promise<void> {
    const ret = await LobbyMgr.inst.rpc_GetAnnouncement('lobby');
    if (!isValid(this.node)) {
      return;
    }

    const layout = this._layout as Layout_UIAnnouncement;
    const pos = layout.maskNode.position;
  ...
  protected onUpdate(dt: number): void {
    const layout = this._layout as Layout_UIAnnouncement;
    if (layout) {
      const pos = layout.lblContent.node.position;
