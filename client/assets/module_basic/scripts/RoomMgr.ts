...
export class RoomEvent {
   ...
   *
   */
  public static PN = 'RoomEvent.PN';

  public static NEW_USER_COMES = 'RoomEvent.NEW_USER_IN_TABLE';
  public static WATCHER_LIST_CHANGED = 'RoomEvent.WATCHER_LIST_CHANGED';
  public static USER_LEAVES = 'RoomEvent.USER_LEAVES';
  public static USER_DATA_CHANGED = 'RoomEvent.USER_DATA_CHANGED';
  public static ROOM_DATA_CHANGED = 'RoomEvent.ROOM_DATA_CHANGED';
}
...
export class RoomMgr {
  ...
  async rpc_LeaveRoom() {
    const ret = await gameNet.callApi('room/ExitRoom', {});
    return ret;
  }

  async backToLobby(silence = false) {
    if (this.isOnline) {
      const ret = await this.rpc_LeaveRoom();
      if (ret.isSucc) {
  ...
  public get isPlayer(): boolean {
    ...
    const user = this.getUser(UserMgr.inst.uid);
  ...
  async enterRoom(params: GameServerAuthParams) {
    const ret = await gameNet.ensureConnected();
    if (!ret.isSucc) {
      return ret;
    }

    const ret2 = await gameNet.joinRoomServer(UserMgr.inst.uid);
    if (ret2.isSucc) {
  ...
  async rpc_Ready() {
    const ret = await gameNet.callApi('room/Ready', {});
    return ret;
  }
  ...
  onNet_UserComesToRoomPush(msg: MsgUserComesToRoomPush) {
    ...
    const user = this.getUser(msg.uid);
    if (!user) {
