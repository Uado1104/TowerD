...
export class HttpLobbyServer {
  ...
  public async callApi<T extends string & keyof ServiceType['api']>(
    apiName: T,
    ...
    const ret = await this._http?.callApi(apiName, req, options);
    if (!ret.isSucc) {
      const filter = this._globalErrorFilters[ret.err.message];
      if (filter) {
