...
export class WebsocketClient {
  ...
  constructor() {
    this.listenMsg('Pong', (msg: MsgPong) => {
      const len = this._latencyCache.length;
      const latency = Math.ceil((Date.now() - msg.timestamp) / 2);
  ...
  createConnection(serverUrls: Readonly<string[]>) {
    ...
    const index = Math.floor(serverUrls.length * Math.random());
  ...
  public callApi<T extends string & keyof ServiceType['api']>(
    apiName: T,
  ...
  public sendMsg<T extends string & keyof ServiceType['msg']>(
    msgName: T,
    msg: ServiceType['msg'][T],
    options?: { timeout?: number; abortKey?: string },
  ): Promise<
    | {
        isSucc: true;
      }
  ...
  private async _connect(): Promise<{ isSucc: boolean; err?: { code?: string; message: string } }> {
    // Connect
    const resConnect = await this._conn?.connect();
