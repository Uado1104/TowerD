...
export class UIAlert extends UIController {
  ...
  public static show(content: string, showCancel?: boolean): UIAlertOptions {
    const opts = new UIAlertOptions() as any;
  ...
  private init(opts: UIAlertOptions) {
    this._options = opts;
    const options = this._options as any as { _title: string; _content: string; _showCancel: boolean };
    const layout = this._layout as Layout_UIAlert;
    ...
    if (!options._showCancel) {
      const pos = layout.btnOK.node.position;
  ...
  protected onCreated(): void {
    ...
    this.onButtonEvent(layout.btnOK, () => {
      this.close();
      const options = this._options as any as { _cbClick: Function; _cbClickThisArg: any };
      if (options._cbClick) {
