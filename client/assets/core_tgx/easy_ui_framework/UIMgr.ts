...
export class UIMgr {
   ...
   * @zh 初始化UIMgr,`不要多次调用`
   *  */
  public setup(uiCanvas: Node | Prefab, maxLayers: number, ly?: Array<string>) {
    if (this._uiCanvas) {
      return;
    }
  ...
  public getUI<T extends UIController>(uiCls): T {
    const allControllers = (UIController as any)._controllers;
   ...
   *  */
  public showUI(uiCls: any, cb?: () => void, thisArg?: any, params?: any): any {
    ...
    const fnLoadAndCreateFromBundle = (bundle: AssetManager.Bundle) => {
      bundle.load(resArr, (err, data) => {
        ...
        const parent = UIMgr.inst.getLayerNode(ui.layer);
