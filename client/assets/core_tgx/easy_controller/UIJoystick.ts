...
export class UIJoystick extends Component {
  ...
  private _cameraSensitivity = 0.1;
  private _distanceOfTwoTouchPoint = 0;
  ...
  protected onLoad(): void {
    ...
    const checkerCamera = this.node.getChildByName('checker_camera').getComponent(UITransform);
    ...
    const checkerMovement = this.node.getChildByName('checker_movement').getComponent(UITransform);
  ...
  setButtonVisible(btnName: string, visible: boolean) {
    const node = this._buttons?.getChildByName(btnName);
    if (node) {
      node.active = visible;
    }
  }
  ...
  onTouchStart_Movement(event: EventTouch) {
    ...
    for (let i = 0; i < touches.length; ++i) {
      ...
      if (!this._movementTouch) {
        ...
        const halfWidth = this._fullscreen.width / 2;
        const halfHeight = this._fullscreen.height / 2;
  ...
  onTouchMove_Movement(event: EventTouch) {
    ...
    for (let i = 0; i < touches.length; ++i) {
      ...
      if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
        ...
        const pos = this._ctrlRoot.node.position;
        ...
        const dirX = ox / len;
        const dirY = oy / len;
        const radius = this._ctrlRoot.width / 2;
        if (len > radius) {
          len = radius;
          ox = dirX * radius;
          oy = dirY * radius;
  ...
  private getDistOfTwoTouchPoints(): number {
    const touchA = this._cameraTouchA;
    const touchB = this._cameraTouchB;
    if (!touchA || !touchB) {
      return 0;
    }
    const dx = touchA.getLocationX() - touchB.getLocationX();
    const dy = touchB.getLocationY() - touchB.getLocationY();
  ...
  private onTouchMove_CameraCtrl(event: EventTouch) {
    ...
    for (let i = 0; i < touches.length; ++i) {
      ...
      if (this._cameraTouchA && this._cameraTouchB) {
        ...
        if (needZoom) {
          const newDist = this.getDistOfTwoTouchPoints();
          const delta = this._distanceOfTwoTouchPoint - newDist;
      ...
      //only one touch, do camera rotate.
        ...
        const dt = touch.getDelta();
        const rx = dt.y * this._cameraSensitivity;
        const ry = -dt.x * this._cameraSensitivity;
  ...
  onKeyDown(event: EventKeyboard) {
    ...
    if (keyCode == KeyCode.KEY_A || keyCode == KeyCode.KEY_S || keyCode == KeyCode.KEY_D || keyCode == KeyCode.KEY_W) {
      ...
      const btnName = this._key2buttonMap[keyCode];
      if (btnName) {
  ...
  onMouseWheel(event: EventMouse) {
    const delta = event.getScrollY() * 0.1;
    //console.log(delta);
    this.emitEvent(EasyControllerEvent.CAMERA_ZOOM, delta);
  }
  ...
  updateDirection() {
    if (this._key2dirMap == null) {
      ...
      this._key2dirMap[KeyCode.KEY_A * 1000 + KeyCode.KEY_W] = this._key2dirMap[KeyCode.KEY_W * 1000 + KeyCode.KEY_A] =
        135;
      this._key2dirMap[KeyCode.KEY_D * 1000 + KeyCode.KEY_W] = this._key2dirMap[KeyCode.KEY_W * 1000 + KeyCode.KEY_D] =
        45;
      this._key2dirMap[KeyCode.KEY_A * 1000 + KeyCode.KEY_S] = this._key2dirMap[KeyCode.KEY_S * 1000 + KeyCode.KEY_A] =
        225;
      this._key2dirMap[KeyCode.KEY_D * 1000 + KeyCode.KEY_S] = this._key2dirMap[KeyCode.KEY_S * 1000 + KeyCode.KEY_D] =
        315;
    ...
    const keyCode0 = this._keys[this._keys.length - 1] || 0;
    const keyCode1 = this._keys[this._keys.length - 2] || 0;
